...
// Transcribe audio - simplified approach for Cloud Run
app.post('/api/transcribe-audio', async (req, res) => {
  try {
    ...
    // For Cloud Run, direct YouTube extraction is unreliable
    // Provide helpful error message with instructions
    return res.status(400).json({ 
      error: 'For the web version, please upload an audio file directly. YouTube URL extraction is not supported in this environment.',
      instructions: [
        '1. Download audio from YouTube using a downloader tool',
        '2. Save as MP3, WAV, or M4A format',
        '3. Use the "Upload Audio File" option instead'
      ]
    });
